...
// Return active news items (no auth required)
// GET /api/news
router.get('/news', async (req, res) => {
    try {
        const result = await db.query(
            `SELECT n.id, n.title, n.content, n.image_url AS imageUrl, n.created_at, u.full_name AS author_name
             FROM news n
...
router.get('/news/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const result = await db.query('SELECT * FROM news WHERE id = $1 AND status = $2', [id, 'active']);
